...
import { serverLog } from "../utils/logger/index";
import { getUserByToken } from "../utils/fetch/user";
		...
		try {
			...
			this.userList = new FPMap({
				setFunction: (key: string, value: User) => {
					serverLog(
						`${chalk.bold.bgCyan(" 用户: ")} ${chalk.bold.cyanBright(value.username)} ${chalk.bold.bgGreen(
							" 连接成功 "
						)}`
					);
			...
			this.roomList = new FPMap({
				...
				deleteFunction: (key: string, value: Room) => {
					serverLog(
						`${chalk.bold.bgYellow(" 房间: ")} ${chalk.bold.yellowBright(key)} ${chalk.bold.bgRed(" 解散了 ")}`
					);
					this.serverBroadcast(SocketMsgType.RoomList, this.getRoomList()); //广播房间列表
					// this.logRoomList();
				},
