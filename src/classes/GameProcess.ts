...
export class GameProcess {
	...
	private initPlayer() {
		this.playersList = this.roomInstance
			.getUserList()
			.map((user) => new Player(user, this.gameSetting.initMoney, getRandomInteger(0, this.mapIndexList.length - 1)));
				...
				await OperateListener.getInstance().once(userId, OperateType.UseChanceCard, (resultArr: any) => {
					...
					if (chanceCard) {
						...
						if (error) {
							...
							this.log(
								`${chalkB.cyanBright(sourcePlayer.getName())} ${chalkB.bgYellow(
									" 使用了机会卡: "
								)} ${chalkB.yellowBright(chanceCard.getName())}`
							);
	...
	private gameOverCheck() {
		...
		switch (gameOverRule) {
			case GameOverRule.Earn100000:
				if (
					this.playersList.some((player) => player.getMoney() >= 100000) ||
					(this.playersList.length > 1 && this.playersList.filter((player) => !player.getIsBankrupted()).length <= 1)
				)
					this.gameOver();
				break;
		}
	...
	public destroy() {
		...
		this.playersList.forEach((p) => {
			OperateListener.getInstance().removeAll(p.getId());
		});
